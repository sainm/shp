...
ESCAPES:
	'\\' (
		...
		| 'x' HexDigits HexDigits
		| .
	);
STRING_CHARACTERS: ~["\\\n$]+;
QUOTED_STRING_VARIABLE_REFERENCE:
	VARIABLE_REFERENCE -> type(VARIABLE_REFERENCE);
QUOTED_STRING_SUBSHELL:
	'$(' -> pushMode(DEFAULT_MODE), type(SUBSHELL);
QUOTED_STRING_ARITHMETIC_OPEN:
	'$((' -> pushMode(DEFAULT_MODE), type(ARITHMETIC_OPEN);
